...
const {
  ...
  resolveTestDirs: mockResolveTestDirs,
...
const { runValidation, matchesExtensionGlob, runStopHook } =
...
describe("validate - stop trigger", () => {
  ...
  it("skips command when {{test_dirs}} empty after agent dedup (with skipped_reason)", () => {
    ...
    expect(logEntry.event.results[0].skipped_reason).toBe(
      "no test dirs after deduplication",
    );
  });
  ...
  it("timeout kills process and marks timed_out", () => {
    mockReadProjectConfig.mockReturnValue({
      validation: {
        stop: [{ name: "slow", command: "sleep 999", timeout_ms: 100 }],
      },
    });
