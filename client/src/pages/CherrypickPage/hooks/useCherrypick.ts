...
function toggleId(set: Set<number>, setFn: (s: Set<number>) => void, id: number) {
...
function toggleAll(exchanges: ConversationExchange[], selected: Set<number>, setFn: (s: Set<number>) => void) {
  const allSelected = exchanges.every((ex) => selected.has(ex.id));
  setFn(allSelected ? new Set() : new Set(exchanges.map((ex) => ex.id)));
}

export function useCherrypick(): { state: CherrypickState; actions: CherrypickActions } {
  ...
  const handlePreview = useCallback(() => {
    if (pageState.phase !== 'edit') return;
    const pruned = pruneConversation(pageState.parsed.raw, deleteMainIds, deleteTangentIds);
  ...
  const handleDownload = useCallback(() => {
    ...
    const blob = new Blob([JSON.stringify(pageState.pruned, null, 2)], { type: 'application/json' });
  ...
  const goBackToEdit = useCallback(() => {
    if (pageState.phase !== 'preview') return;
    setPageState({ phase: 'edit', parsed: pageState.parsed, fileName: pageState.fileName });
  }, [pageState]);
  ...
  return {
    ...
    actions: {
      ...
      toggleTangentId: (id) => toggleId(deleteTangentIds, setDeleteTangentIds, id),
      toggleAllMain: (exchanges) => toggleAll(exchanges, deleteMainIds, setDeleteMainIds),
      toggleAllTangent: (exchanges) => toggleAll(exchanges, deleteTangentIds, setDeleteTangentIds),
      goBackToEdit,
    },
  };
}
