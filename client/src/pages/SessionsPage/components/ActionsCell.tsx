...
export function ActionsCell({ session }: { session: SessionWithStatus }) {
  ...
  const actions: ActionItem[] = [
    ...
    },
    {
      ...
      action: () => navigator.clipboard.writeText(displayName),
    },
    {
      id: "copy-pid",
      text: "Copy PID",
      action: () => navigator.clipboard.writeText(String(session.pid)),
    },
    {
      id: "delete",
      text: "Delete session",
    ...
    >
      ...
      >
        <Alert type="warning">
          ...
          <strong>{displayName}</strong> (PID {session.pid}). This action cannot
          be undone.
        </Alert>
      </Modal>
    </span>
