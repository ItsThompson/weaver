...
import {
  webhook, mockFetch, mockReadConfig, mockParseLogFile,
...
describe('buildWebhookPayload (advanced)', () => {
  it('returns null fields for agentSpawn', () => {
    const payload = webhook.buildWebhookPayload('sess-1', 'agentSpawn', 'starting', 'my-project', TEST_SESSION, []);
  ...
  it('extracts prompt for userPromptSubmit', () => {
    ...
    const payload = webhook.buildWebhookPayload('sess-1', 'userPromptSubmit', 'processing', 'my-project', TEST_SESSION, events);
  ...
  it('extracts tool context for preToolUse', () => {
    ...
    const payload = webhook.buildWebhookPayload('sess-1', 'preToolUse', 'running_tool', 'my-project', TEST_SESSION, events);
  ...
  it('stringifies tool_response for postToolUse', () => {
    ...
    const payload = webhook.buildWebhookPayload('sess-1', 'postToolUse', 'processing', 'my-project', TEST_SESSION, events);
