...
export const mockReadConfig = jest.fn<() => Promise<{ config: WeaverConfig; warnings: string[] }>>();
...
export function makeEvent(name: string, extra: Record<string, unknown> = {}): HookEvent {
  return { timestamp: '2026-01-01T00:00:00Z', event: { hook_event_name: name, cwd: '/tmp', ...extra } };
}

export function configWith(url: string, format: 'simple' | 'advanced' = 'simple') {
