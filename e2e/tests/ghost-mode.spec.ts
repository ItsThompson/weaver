import { test, expect, MAIN_ENTRY, REPO_ROOT, SERVER_URL, killPort } from "../fixtures/app.js";
...
test.describe("ghost mode", () => {
  ...
  test("enable ghost mode", async ({ electronApp, page }) => {
    await electronApp.evaluate(() => (global as any).__weaverTest.setGhostMode(true, 0.5));
  ...
  test("disable ghost mode", async ({ electronApp, page }) => {
    await electronApp.evaluate(() => (global as any).__weaverTest.setGhostMode(true, 0.5));
    await electronApp.evaluate(() => (global as any).__weaverTest.setGhostMode(false, 0.5));
  ...
  test("ghost + toggle interaction — show restores ghost opacity", async ({ electronApp, page }) => {
    await electronApp.evaluate(() => (global as any).__weaverTest.setGhostMode(true, 0.5));
    // Hide
  ...
  test("ghost mode from config", async ({ tmpDir }) => {
    ...
    try {
      ...
      const state = await app.evaluate(() => (global as any).__weaverTest.getState());
      expect(state.ghostEnabled).toBe(true);
